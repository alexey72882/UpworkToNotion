...
- **Filters page black-stroke flash on load (fixed 2026-08-16)**: the Filters page conditionally swaps a full skeleton subtree (`if (loading) return <skeleton/>`) for the real content. When the content subtree first paints, it shows **one high-contrast frame** — dark chips at full opacity + a dark ring around the card — before styles settle (verified with a CDP screencast: the flash is exactly the mount frame; the frame ~90ms later is clean). `getComputedStyle` on the card at those frames shows no black outline/border/box-shadow, so it's a one-frame paint/FOUC artifact of the large subtree swap, **not** a single CSS rule. (An earlier fix guessing daisyUI's `.card:has(> :checked){ outline: 2px solid }` selectable-card rule did not resolve it — that rule needs a *direct*-child checked input, which the nested filter checkboxes never are.) Fix: fade the loaded content in so the flash frame is invisible — `content-fade-in` class on the loaded content root + `@keyframes contentFadeIn { from{opacity:0} to{opacity:1} }` (0.25s) in `globals.css`. To reproduce/verify, temporarily seed the loaded state with active filters + a delayed `setLoading(false)`, then CDP-screencast the mount frames.
