...
- **Filters page black-stroke flash on load (fixed 2026-08-16)**: on load the Filters card briefly showed a ~2–3px near-black rounded ring, then cleared. Root cause: daisyUI 5's `.card` has `transition: outline .2s` plus outline rules for focus (`.card:focus-visible { outline-color: currentColor }`) and selectable cards (`.card:has(> :checked) { outline: 2px solid }`). During the skeleton→content swap the card transiently gets an outline; when it clears, the `.2s` transition **animates the outline width down to 0**, so a 1-frame blip renders as a ~200ms fading ring in `--color-base-content` (#18181b). Confirmed by a CPU-throttled (20×) CDP probe: at the flash frame `outlineWidth` is `3px` / color `#18181b`, but `document.activeElement` is `<body>` and the card matches neither `:focus`/`:focus-visible`/`:focus-within` nor `:has(> :checked)` — i.e. it's the transition remnant, not a live outline state. **Earlier wrong guesses (do not repeat):** `.card:has(> :checked){outline:none}` (needs a *direct*-child checked input — the filter checkboxes are nested, so it never matched) and a content fade-in (masked it in headless but not in prod). **Fix:** `.card { outline: none !important; }` in `globals.css` — our cards are presentational, never interactive, so no card ever needs an outline. To reproduce/verify: temporarily seed the loaded state with active filters + a delayed `setLoading(false)`, launch headless Chrome with `--remote-debugging-port` + `Emulation.setCPUThrottlingRate {rate:20}`, and screencast the mount frames (throttling stretches the ~200ms fade so screenshots catch it).
